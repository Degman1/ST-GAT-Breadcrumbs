    ...
    for param_set in param_combinations:
        ...
        for fold, train_ratio in enumerate(train_ratios):
            ...
            for epoch in range(epochs):
                ...
                with torch.no_grad():
                    train_rmse, train_mae, train_mape, _, _, attention_matrix = models.trainer.eval(model, device, train_dataloader, config, "Train")
                    val_rmse, val_mae, val_mape, _, _, _ = models.trainer.eval(model, device, val_dataloader, config, "Valid")
                ...
                models.trainer.writer.add_scalar(f"MAE/val", val_mae, epoch)
            ...
            print(f"Achieved validation MAE of {min_val_mae} over {stopped} epochs for fold {fold}")
